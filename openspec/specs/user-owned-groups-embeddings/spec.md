# user-owned-groups-embeddings Specification

## Purpose

Introduce a single-owner `Group` entity that organizes `Meeting` rows and build group-scoped semantic indexing over meeting transcript/summary text. The indexed content is semantically retrievable by the owner through an internal, authorization-enforcing retrieval contract. The deliverable is the group entity, the asynchronous indexing lifecycle, and the retrieval seam — MCP server, in-product chat, and generic artifact models are out of scope.
